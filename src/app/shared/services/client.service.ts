...
export class ClientService {
  ...
  public getAll(): void {
    this.http.get<Client[]>(`${this.url}`, {headers: this.getHeaders()}).subscribe({
  ...
  public addClientToRoom(clientId: number, roleName: string): void {
    this.http.post(`${this.url}/add-room/${clientId}/with-role/${roleName}`, roleName, {headers: this.getHeaders()}).subscribe({
